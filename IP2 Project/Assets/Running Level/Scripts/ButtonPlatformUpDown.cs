...
public class ButtonPlatformUpDown : MonoBehaviour
{
	...
	private float useSpeed; //Sexier code...
	...
	void Awake()
	{
		originalPosition = movingPlatform.transform.position.y;
	...
	void TriggerPlatform()
	{
		...
		if (platTriggered)
		{
			...
			if (originalPosition - movingPlatform.transform.position.y > distance)
				useSpeed = directionSpeed;
			else if (originalPosition - movingPlatform.transform.position.y < -distance)
				useSpeed = -directionSpeed;

			//Translate the platform 
			movingPlatform.transform.Translate (0, useSpeed * Time.deltaTime, 0);
		} 
	}
}
